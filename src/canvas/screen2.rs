...
pub fn test() {
    ...
    let (w, h) = (151, 40);
    let mut screen = Screen::<_, Damaged>::new(
        Layer::new(
            (0, 0),
    ...
    let mut frame = screen.frame_mut((10..20, 10..20)).unwrap();
    frame.paint(('a', Background(RED)));
    frame.render().unwrap();
    frame.flush().unwrap();

    let mut frame = frame.frame_mut((1..5, 1..5)).unwrap();
    frame.paint(('b', Foreground(BLUE), Bold, Italic));
    frame.render().unwrap();
    frame.flush().unwrap();
}
